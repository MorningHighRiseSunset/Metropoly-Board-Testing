// Token Selection UI Module with Babylon.js

const BabylonTokenSelection = {
    selectedToken: null,
    spawnedTokens: [],
    currentTile: null,
    scene: null,
    ...
    createTokenPanel() {
        ...
        BabylonTokenLoader.getAllTokenNames().forEach(tokenName => {
    ...
    spawnToken() {
        ...
        const tokenModel = BabylonTokenLoader.getToken(this.selectedToken);
        if (!tokenModel) {
        ...
        // Clone the token
        const clonedToken = BabylonTokenLoader.cloneToken(this.selectedToken, this.scene);
        if (!clonedToken) {
        ...
        clonedToken.position.x = this.currentTile.position.x;
        clonedToken.position.y = 0.5 + offsetZ * 0.1;
        clonedToken.position.z = 0.5 + offsetZ * 0.1;
        clonedToken.scaling = new BABYLON.Vector3(0.8, 0.8, 0.8);
    ...
    removeToken(tokenId) {
        ...
        if (this.tokenObjects[tokenId]) {
            this.tokenObjects[tokenId].mesh.dispose();
...
// Export to window
window.BabylonTokenSelection = BabylonTokenSelection;
